	...
	<main>
		...
		<div class="content18">
					...
					<table>
						...
						<tbody>
							<?php foreach ($tak->leaders as $leader): ?>
								<?php if ($leader->takleiding): ?>
									<tr>
										<td>
											<?= $leader->firstname.' '.$leader->name ?>
											<?= ($leader->show_nick) ? ' ('.$leader->nickname.')' : '' ?> - TL
										</td>
										<td><?= $leader->address ?><br><?= $leader->zip.' '.$leader->city ?></td>
										<td><a href="mailto:<?= $leader->username ?>"><?= $leader->username ?></a></td>
										<td><a href="<?= preg_replace('/\D/', '', $leader->gsm) ?>"><?= $leader->gsm ?></a></td>
									</tr>
								<?php endif ?>
							<?php endforeach ?>
							<?php foreach ($tak->leaders as $leader): ?>
								<?php if (!$leader->takleiding && $leader->active): ?>
									<tr>
										<td>
											<?= $leader->firstname.' '.$leader->name ?>
			...
			<div class="takken" id="leiding">
				...
				<div class="row">
					<?php foreach ($leaders as $leader): ?>
						<?php if (!$leader->grl && $leader->active): ?>
							<div class="one-half leider">
								<p class="pull-left">
									<?= $leader->firstname ?> <?= $leader->name ?>
									<?= ($leader->show_nick) ? '<br>(' . $leader->nickname . ')' : '' ?>
